...
def predict_test(export_path, model):
    ...
    for pred_epoch in range(TEST_AUG_EPOCH):
        ...
        # model.eval()
        with torch.no_grad():
            for batch_i, (imgs1, imgs2, idx) in enumerate(loader_test):
                for i, imgs in enumerate([imgs1, imgs2]):
                    imgs = imgs.to(device)
                    feature, output = model(imgs)
...
def trainer(fold_i):
    # Model
    if opt.no_metric:
        model = get_model(opt.model, NUM_CLASSES, False)
    else:
        model = get_model(opt.model, NUM_CLASSES, True)
    model.to(device)
    ...
    for epoch in range(start_epoch, start_epoch + opt.epoch):
        ...
        train_accuracies = []
        for i in range(2):
            for batch_i, (imgs, labels) in enumerate(loader[i]):
        ...
        if epoch % opt.evaluation_interval == 0:
            ...
            with torch.no_grad():
                for val_epoch in range(VALID_AUG_EPOCH):
                    for i in range(2):
                        for imgs, labels in loader_val[i]:
                            imgs = imgs.to(device)
...
if __name__ == "__main__":
    ...
    result_path = Path(f'results/{opt.model}{opt.img_size}{"_fc" if opt.no_metric else ""}')
    result_path.mkdir(exist_ok=True)
    if opt.full_train:
        (result_path/'full').mkdir(exist_ok=True)
    log_path = Path(f'logs/{opt.model}_{opt.img_size}_{opt.experiment}{"_full" if opt.full_train else ""}{"_nometric" if opt.no_metric else ""}')
    ...
    if opt.cv in [0, 1]:
        ...
        ds = [ImagesDataset(df_train, str(data_path), mode='train',
                            site=[site], transforms=img_t, mixup=opt.mixup) for site in [1, 2]]
        if df_val is not None:
            ds_val = [ImagesDataset(df_val, str(data_path), mode='train',
                                    site=[site], transforms=img_tt) for site in [1, 2]]
        ds_test = ImagesDataset(df_test, str(data_path), mode='test',
                                site=[1,2], transforms=img_tt)
        idxs = df_test.index
